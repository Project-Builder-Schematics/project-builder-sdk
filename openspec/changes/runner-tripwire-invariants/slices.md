...
- [ ] S-001.7 Convert this slice's message assertions to whole-verbatim (REQ-CST-06.1) + add the standing scan forbidding `toContain` on tripwire messages (later slices must not violate it). **PARTIAL — see apply-progress**: every NEW assertion this slice added is whole-verbatim by construction (no new `toContain` landed); the 3 pre-existing S-003-tier assertions this slice's own rule-rename touched were converted to whole-verbatim. The bulk pre-existing `toContain` inventory (~44 of the ~47 named in design.md §6's inventory) across `fit-42-*`/`fit-23-*`/`fit-46-*` remains UNCONVERTED, and the standing anti-`toContain` scan is NOT yet added — both deferred, flagged for a dedicated follow-up pass (see apply-progress "Deferred work"). **Scope (plan-verify iteration-1 amendment, gap 8)**: the scan is scoped to TRIPWIRE-MESSAGE assertion sites — `test/fitness/fit-42-runner-closure-integrity.test.ts`, `test/fitness/fit-42-runner-closure-integrity.negative.test.ts`, `test/fitness/fit-23-publish-workflow-guard.test.ts`, `test/fitness/fit-46-publish-sequence-integrity.test.ts` (the fit-42/fit-23/fit-46 family this change touches) — NOT repo-wide (a repo-wide scan would flag unrelated `toContain` usage this change has no acceptance criterion over). See "Build Order" below — batch 2 builds SEQUENTIALLY (S-001 → S-003 → S-004, plan-verify
