...
### Tasks
- [x] S-005.1 `test/docs/runner-integrity-docs.test.ts`: counts read from live derivation, frozen literals removed (REQ-DLV-01.1). Added `findStaleCountClaims(markdown, closureFileCount)`: renders a fixed set of frozen PROSE templates (mirroring this file's own pre-existing frozen-string convention) against `deriveRunnerClosure`'s live `nodes.length` (23) and `+ 1` (24, the total manifest entry count including `package.json`) — no `23`/`24` literal appears anywhere in the new describe block. All 4 doc claims (`lists N files:`, `Hashing our own N files`, `verifying N digests`, `entry #N because`, `the N emitted .js files`, `N closure files plus`, `all N closure files:` — 7 template renders across 2 count families) verified against the real doc.
- [x] S-005.2 Stale-count red-proof (REQ-DLV-01.2). Two red-proofs, one per count family (total-entry, closure-file), each mutating exactly one real doc phrase (`.replace`) and asserting `findStaleCountClaims` returns exactly one mismatch, naming the claim's label and the live value it should have matched — proven exact (not merely non-empty), so the function is shown to discriminate correctly-rendered claims from stale ones rather than vacuously flagging everything.
- [x] S-005.3 `docs/runner-integrity-invariants.md`: Constraint-4 section describes admission, not deny-scan; enumeration promise matches enforcement exactly. Replaced "Any `createRequire` reference in a closure file is a violation... The same scan covers `eval`, `new Function`, `node:vm`, `Bun.plugin` and `process.binding`" (5 of the register's 11 members, deny-scan-list framing) with a default-DENY capability-admission framing naming the full, current 11-member register (`eval`, `Function`, `createRequire`, `Bun.plugin`, `process.binding`, `node:vm`, `node:child_process`, `node:worker_threads`, `WebAssembly`, `module.register`, `module.registerHooks`) plus the S-002 exemption-forfeiture rules (aliasing, re-export laundering, anchor drift). Re-ran the pre-existing REQ-IID-01 structural tests (heading/`enforced-by` parsing) after the edit — all still pass; the heading text itself was left unchanged, only the body paragraph.
