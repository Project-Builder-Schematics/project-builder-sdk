...
Pins the SEMANTIC contract of the by-reference copy directive — the wire encoding
itself (new op vs. `copy` source discriminator) is an OPEN DESIGN DECISION (obs #915
ruling 19); this domain is deliberately WIRE-SHAPE-AGNOSTIC. Every requirement below is
phrased in terms of "the by-reference directive" without presuming its exact `op`
value or field layout — `sdd-design` resolves the encoding and MUST satisfy every REQ
here regardless of which alternative wins.
...
containment at apply time — the ONLY real security control (obs #915 ruling 10).
...
(Previously V2: phrased as an in-fake `#requireExists`/FAKE-06 simulation obligation —
an implementation leak; reworded at V3 to the end-to-end WHAT.)
