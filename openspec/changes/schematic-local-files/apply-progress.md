...
RED evidence is condensed above (see `## S-000.4`/`## Outer loop` rows) — the outer e2e test's four original assertions failed for the four DISTINCT structural reasons quoted, confirmed via the actual `bun test` run before any S-000 code existed (see commit history: e2e test file predates the `src/core/context.ts`/`src/scaffold/` commits).
