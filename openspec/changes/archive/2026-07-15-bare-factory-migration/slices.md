...
## §21. Build-Iteration-1 Resolutions Addendum (orchestrator, 2026-07-14 — slices rev 4)

**R-9 (apply S-000 discovery + verify-in-loop-1 Finding 1 — 5 unclaimed old-convention files, RULED)**: `test/fake/copyin-fidelity.test.ts`, `test/scaffold/batch-cap-chunk.test.ts`, `test/scaffold/classify-transport.test.ts`, `test/scaffold/scaffold-fake.test.ts`, and `test/e2e/author-emulation/ir-transcript.test.ts` use the identical to-be-migrated wrapped calling convention but were absent from every slice's Covers/Failing-first list (design's "exactly 4 harness files" inventory came from a narrow `test/fake`-only search). They FOLD INTO S-002 — same fix shape as its existing 4 harness files (bare test factories + `runFactoryForTest` options bag). They do not self-heal at S-004 (none route through `SCENARIOS`). S-002's file set is therefore 4 + 5 = 9 consumer files. Coverage, REQ set, and build order unchanged; this is an inventory correction, not new scope — the signed spec already mandates zero wrapped-shape callers repo-wide by S-006.

**R-10 (verify-in-loop-1 Finding 2 — S-000 runtime evidence gap, RULED)**: REQ-ATH-01.4 / REQ-ATH-17.1 / REQ-ATH-17.2 need EXECUTED behavioral tests (options-bag `seed` reaches `ContractFake`; `packageDir` present → schema validation fires / `AuthoringError reason:"invalid-input"`; `packageDir` absent → byte-identical untyped run). A type-only `void`-wrapped proof does not discharge a behavioral GIVEN/WHEN/THEN. This is S-000 completion work (Executor fix, in-loop iteration 1), not new scope.

**R-11 (verify-in-loop-4 adjudication, orchestrator 2026-07-14 — SUPERSEDES §20 R-3)**: `scratchFactoryRunner` does NOT stay a direct `defineFactory` caller — R-3's premise (scratch runner as outermost driver) was invalidated by S-004's own bare-`FactoryRunner` obligation: the outer `deps.client` disappears from the signature and `Session.#client` is a true private field, so a nested `defineFactory` would split directives away from the outer capture. Accepted redesign: re-anchor `RunContext.packageAnchors` for the scratch dir's lifetime (restored in `finally`), reserved-name check replicated via the same exported utilities. Behavior preservation PROVEN (corpus regen git-clean no-op). Bounded drift risk (skipped `validateAtRunBoundary`, hardcoded ceiling, dropped scan-failure wrapper) — REGISTER FOLLOWUP at archive: lockstep pin test or shared helper between `defineFactory`'s gate and the scratch replica.
