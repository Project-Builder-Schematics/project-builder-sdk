...
under the prior containment regime," never "accepted." There is no narrowing for a literal `..`
...
re-homed from the retired containment family's own emit-time guard; (c) the emitted-source-path-never-
...
- GIVEN a literal-string source such as `from: "../shared/base.txt"`, driven once per
  verb (three cases)
- WHEN called under BOTH the prior containment regime and this screen
...
### REQ-IPF-02: SDK Emit-Time Lexical Destination Guard (re-homed from the retired containment family's own emit-time guard)
...
|---|---|---|
| security (IR shape, explicitly NOT a containment boundary) | REQ-IPF-01, REQ-IPF-02, REQ-IPF-03 | Yes — re-homed from the retired containment family; scoped to lexical well-formedness, never realpath-based containment |
