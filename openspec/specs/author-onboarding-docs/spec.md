...
### REQ-AOD-03: Verb Reference with Read-Trichotomy Rule

A verb-reference doc MUST document all seven author verbs (`create`, `modify`, `remove`,
`rename`, `move`, `copy`, `copyIn`) and the `find().read()` read-trichotomy rule. (Archive-time
correction: `stage-6-release-shape`'s plan-time text named six verbs — `main` gained `copyIn` via
`schematic-local-files`/ADR-0043 before this spec synced; seven is the current floor, not a
superset exception.)

#### Scenario REQ-AOD-03.1: All seven verbs and the read-trichotomy rule are documented

- GIVEN the verb-reference doc
- WHEN scanned for each of the seven verb names and a read-trichotomy explanation
- THEN all eight elements are present
