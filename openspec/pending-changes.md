...
|---|---|---|---|---|---|
| **force-removal followup (design §4.8 / engine handoff-2)**: Remove `force?: boolean` from the create directive type in `src/core/wire.ts` (verify exact file/line) + correct `docs/create-templates.md` §"Overwrite behavior (force)" from pass-to-overwrite to fail-closed with PC-CREATE-01 as the only reopen path — author-facing, S, own change | feature-cleanup | S | — | — | — |
...
| Description | Type | Size | Gating? | Stage |
|---|---|---|---|---|
| **BRC-02 — engine ceiling re-derivation + TOCTOU closure**: the engine MUST re-derive its OWN containment ceiling for a by-reference (`copyIn`) source — open-then-fstat, never trusting the SDK-resolved `packageDir`. Anchor pin (ADR-0043 rev 2, seam clause S2): the emitted `copyIn.from` is relative to the RESOLUTION anchor (`packageDir`) — the engine resolves `from` against its OWN re-derived ceiling, independent of the SDK's own anchor. **RE-CITED 2026-07-28 (`inline-collection-marker`, ADR-0077)**: the SDK-side `packageRoot`/containment-ceiling anchor this row used to mirror is RETIRED — `packageDir` is now the SDK's sole anchor (`package-dir-run-anchor` REQ-MFB-01); this row's gating is UNCHANGED, only the anchor-distinction language it cited is stale. **ADVANCE-NOTE 2026-07-15 (`stdio-engine-client`)**: no change to this row's gating — noted alongside the wire spec's Bridge Contract/build-target pins for grouping | security | M | Blocking engine apply-pass for `copyIn`/by-reference `scaffold` entries | **engine repo, cross-repo flag (with PC-PROTO-01)** |
| **BRC-08 — non-canonical path-form rejection + single-pass literal-token render**: the engine MUST reject non-canonical path forms (UNC/device/reserved-DOS/drive-relative) for BOTH the by-reference source and the rendered destination, and MUST render a `create` directive's `pathTemplate` single-pass (substituted token values are literal segments, never re-scanned as further template syntax). Canonical-form hardening is explicitly OUT of SDK scope. **RE-CITED 2026-07-28 (`inline-collection-marker`, ADR-0077)**: this row's premise that the SDK case-folds on case-insensitive platforms is STALE — the SDK no longer disk-canonicalizes OR case-folds package-local sources at all (no ceiling comparison exists to case-fold); `ir-path-well-formedness`'s Windows-out-of-scope note is the current citation for what the SDK's lexical screen does NOT cover, and this row remains the engine-side complement for all of it. **ADVANCE-NOTE 2026-07-15 (`stdio-engine-client`)**: the frame contract (length-prefix, cap) this row's blast radius sits behind is now pinned in `docs/engine-sdk-wire-spec.md` — no change to this row's gating | security | M | Blocking engine apply-pass for `copyIn`/`scaffold`/`create({templateFile})` | **engine repo, cross-repo flag (with PC-PROTO-01)** |
| **PRC-06 — post-render destination containment**: the engine MUST enforce destination containment POST-RENDER — after token substitution, the final on-disk destination path must be re-validated against the target workspace boundary. SDK-side `validateDestinationLexical` (`ir-path-well-formedness` REQ-IPF-02, re-cited 2026-07-28 — successor to the retired `package-root-containment` REQ-PRC-09) is lexical-only, pre-render, and cannot see rendered token values — it is DX/attribution only, never the security control for a rendered destination. **ADVANCE-NOTE 2026-07-15 (`stdio-engine-client`)**: unaffected by the wire contract directly; noted alongside BRC-02/BRC-08 for grouping — no change to this row's gating | security | M | Blocking engine apply-pass for `scaffold`/`create` | **engine repo, cross-repo flag (with PC-PROTO-01)** |
...
| CCF-5 | **SDK-plane `copyIn` source-twin unit-coverage verification** (spec Followup 1, descoped by owner ruling 2). The corpus deliberately authors NO `m2-copyin` source-side rejection cases (containment-escape, missing-source, dir-source — all `(1,null,null,[])` + empty transcript, authoring-rejected pre-emit). Followup: confirm the SDK unit suites (`by-reference-copy-wire` REQ-BRC-06, `package-root-containment` REQ-PRC-04/07) actually exercise all three source-side rejection planes with that exact outcome shape; add any missing case THERE (never in the corpus — deliberate descope, not a coverage gap). | test-coverage | XS | Next SDK unit-suite touch (or at un-hold review) | TBD |
...
**ESCALATED, not approved** — two blind judges over two rounds found the change's build-time
tripwires defective; fix iteration 1 closed three findings, Round 2 reopened two of them, fix
iteration 2 closed those two properly, and the owner stopped there. **Nothing below affects
`dist/runner-manifest.json` itself**, which shipped in PR #50 and is correct, deterministic,
digest-verified and fail-closed — the engine's `PC-RUN-01` is unblocked. Every row concerns the
guards against *future* drift.

**Read the meta-finding first**, because it reframes most of the rows: Constraint 4 is enforced
by an **AST-shape scanner**, and shapes have a long tail. This cycle spent ~25 review-and-fix
rounds closing spellings that judges imagined; the only fix with no obvious next variant was the
last one, which *inverted the invariant* instead of adding a case. Treat the Constraint-4 rows
below as evidence for a different mechanism, not as a queue of shapes to patch.

| Description | Type | Size | Gating? | Stage |
|---|---|---|---|---|
| **Constraint 4 wants a structural invariant, not a shape scanner** (meta-finding, judgment-day Round 2 assessment): the deny-scan enumerates AST shapes (`createRequire` text, alias bindings, resolve-only call shape, computed member access, namespace form). Two judging rounds closed five spellings and neither round could establish the set is closed. R2-1's fix demonstrates the alternative — require the anchor to *prove* a shape (exactly one unaliased binding) rather than searching for one among whatever it imports. Rows R2-5, R1-7, R1-16 and R1-17 below are all the same class and would likely dissolve under a re-designed mechanism. **Decide the mechanism before patching the rows** | architecture | M | — | **own scoped change, before further Constraint-4 work** |
| **R2-3** The invalid-`version` failure renders under the `unreadable-file` rule, which misdescribes it: `package.json` was read fine, its `version` field is unusable. Needs its own `ViolationRule` so the message names the real cause | bugfix | XS | — | next `generate-runner-manifest.ts` touch |
| **R2-4** `JSON.parse` of `package.json` (`generate-runner-manifest.ts:75`) sits outside the `failClosed` path — malformed JSON throws raw instead of producing a rule-shaped, fail-closed diagnostic. Same fix site as R2-3 | bugfix | XS | — | next `generate-runner-manifest.ts` touch |
| **R2-5** False positive at the anchor: `module.createRequire(u).resolve(s)` — the namespace form the anchor file's own header comment documents — is rejected, because only the named-import binding path grants the exemption. Live nuisance (the comment describes a form the scanner forbids), no security impact | bugfix | XS | — | next `derive-runner-closure.ts` touch |
| **R2-6** Bundler-disjointness normalisation still escapable: `.//dist/x` (Judge A) and `--outdir .` (Judge B) survive the `./`-strip + `posix.normalize` in `normaliseForComparison` (`test/support/closure-integrity-checks.ts:128-130`). Round-1 finding 2 was fixed for `./`, trailing `/` and `--outfile`; these two spellings remain | bugfix | S | — | next `closure-integrity-checks.ts` touch |
| **R1-5** A post-derivation throw exits non-zero while leaving a **stale** manifest on disk — fail-closed is violated in the window between derivation and write | bugfix | S | — | next `generate-runner-manifest.ts` touch |
| **R1-6** `writeFileSync` is not atomic: an interrupted write leaves a truncated manifest that the engine would read as authoritative. Write-temp-then-rename | bugfix | S | — | next `generate-runner-manifest.ts` touch |
| **R1-7** Computed member access (`globalThis["eval"]`, `p["binding"]`) evades the Constraint-4 deny set, which matches only identifier text and dotted member expressions. **Same class as the meta-finding** | security | S | — | gated on the Constraint-4 mechanism decision |
| **R1-8** A directory specifier entering the closure is misdiagnosed as `unreadable-file` rather than named as a directory | bugfix | XS | — | next `derive-runner-closure.ts` touch |
| **R1-9** The BOM non-vacuity guard asserts `> 0` where its CRLF sibling pins `=== 23`; the weaker form degrades silently if the fixture stops being emitted | test-coverage | XS | — | next `fit-42` touch |
| **R1-10** CST-04.3's non-vacuity check counts **substring** hits, 8 of which come from comments rather than code — the count does not measure what it claims | test-coverage | S | — | next `fit-42` touch |
| **R1-11** `docs/runner-integrity-invariants.md` hardcodes the 23/24 counts as prose, unbound to the derived closure — they drift silently the moment the closure changes. Bind them to the derivation the way `enforced-by:` is already bound | docs | S | — | next docs touch |
| **R1-12** `BPI-04.1` spawns the generator against the **real** `dist/` mid-suite, so a suite run mutates the tree other tests read. Isolate to a scratch tree | test-coverage | S | — | next `fit-42` touch |
| **R1-13** `publishRunSteps` treats YAML job declaration order as execution order — a wrong assumption that happens to hold today | bugfix | XS | — | next packaging-check touch |
| **R1-14** The symlink-escape check is not applied to the **entry** file, only to walked descendants | security | S | — | next `derive-runner-closure.ts` touch |
| **R1-15** `node:`-prefixed specifiers are not validated against `builtinModules`, so `node:nonexistent` classifies as a builtin | bugfix | XS | — | next `derive-runner-closure.ts` touch |
| **R1-16** JSDoc identifiers **are** included in the descendant walk, so `{@link createRequire}` at the anchor is scanned as code. Under the inverted invariant this can no longer steal the exemption slot (consumption requires the resolve-only call shape), so the residue is a false positive, not an evasion — **re-verify under the new mechanism before spending effort** | test-coverage | XS | — | next `derive-runner-closure.ts` touch |
| **R1-17** The bare `Function` identifier ban rejects ordinary JavaScript (`x instanceof Function`, `Function.prototype`) anywhere in the closure. **Same class as the meta-finding** | bugfix | S | — | gated on the Constraint-4 mechanism decision |
| **R1-18** `srcPathFor`/`srcOf`'s extension rewrite is lowercase-only (`.JS` not handled) | bugfix | XS | — | next touch of either |
| **Architecture baseline refresh**: `ts-morph` gained a third importer — build-time, under `scripts/`, and **not shipped** (it is absent from the runner closure by construction). `architecture_impact: additive`, so refresh is prompted rather than mandatory; the baseline currently describes two importers | docs | XS | — | **next `/sdd-architecture refresh`** |
| **`node:vm` altitude fold** (simplify gate, skipped with stated reason): `node:vm` is special-cased in `classifySpecifier` while the sibling primitives live in `denyScan`'s deny set — the "closure may RESOLVE, never EXECUTE" invariant is enforced by two mechanisms instead of one. Skipped at the final gate because CST-06 pins violation message shapes and this sits in the security-critical classifier. **Folds naturally into the Constraint-4 mechanism decision** | architecture | S | — | gated on the Constraint-4 mechanism decision |
| **`react-conformance.test.ts` declares no timeout** and runs 6.16–6.29 s against Bun's default 5000 ms per-test ceiling — the cause of this cycle's intermittent single-test failures. Pre-existing and owned by the react dialect, not by this change | bugfix | XS | — | **next react-dialect touch** |
| **Two spec-wording deviations awaiting an owner ruling** (both shipped, both deliberate, neither reopened the signed spec): (1) `REQ-RMD-05.1` says the manifest bytes carry no username; the shipped test asserts no username **path segment**, because CI's user is literally named `runner` and a bare substring test is undiscriminating against `dist/bin/pbuilder-runner.js`. (2) `REQ-RMD-01.2` is **satisfied-in-intent** — Bun's default `Intl.Collator` resolves `en-US` regardless of `LC_ALL`/`LANG`/`LC_COLLATE`, so the cross-process locale scenario cannot discriminate the mutation it names; intent is covered by `RME-05.2`'s pinned pairs. Both need the spec text amended to match what is enforced, or the enforcement changed to match the text | docs | XS | — | **next `runner-integrity-manifest` spec touch** |

## From `inline-collection-marker` archive (2026-07-29) — verify-final, judgment-day, and outcome-check followups

Verify verdict `pass-with-followups` (7 followups F-1..F-7; F-4/F-5 DISCHARGED at this
archive — the same-commit spec-sync obligation and the sensitive-areas.md row are both
now landed). Judgment-day APPROVED across 3 rounds (13 confirmed real fixes: G1-G8 round
1, F1-F5 round 2, F1-F8 round 3 — see `apply-progress.md`); the remaining rows below are
the suggestion/theoretical pool judgment-day raised but did not fix, plus the reckoning's
own outcome-check.

| Description | Type | Size | Gating? | Stage |
|---|---|---|---|---|
| **Outcome-check** (steward reckoning, `delivered-pending-activation`): re-open the outcome question once (a) `openspec/pending-changes.md` rows 268-270 (BRC-02/BRC-08/PRC-06) are DISPOSITIONED by the engine team absorbing `CONFORMANCE-CORPUS-HANDOFF.md` Addendum 3, and (b) a real inline-collection project runs against a published `0.0.0-dev.<sha>` build, confirming the temp-dir equivalence this change asserts by construction (D1 + D3, `outcome-verdict.md` §4) | other | — | Next consumer install of a published build, or engine REQ-BRC-08 work | **deferred-pending-activation, cross-repo flag** |
| **F-1**: Add the missing per-verb integration legs for REQ-IPF-01.2/.6 and REQ-PSH-01.1/02.2/02.4/03.1, **or** amend the spec's ×3-verb phrasing to name the structural substitute (`fit-45` call-site pin) it relies on | test-coverage | S | — | **next scaffold-family touch** |
| **F-2**: Add the zero-stat/read assertion at the `templateFile` and `copyIn` verb boundaries (REQ-IPF-01.1) | test-coverage | XS | — | **next scaffold-family touch** |
| **F-3**: Re-implement `fit-43` clause (d) as the spec's per-symbol allowlist; drop the blanket `*.test.ts` exclusion | refactor | S | — | **next fit-43 touch** |
| **F-6**: Raise `test/docs/quickstart-docs.test.ts` REQ-AOD-11's per-test timeout (pre-existing flake under bun's default 5s ceiling, unrelated to this change) | bugfix | XS | — | **next quickstart-docs touch** |
| **F-7**: Remove the superseded comment paragraph above `withOptionalLocator` in `walk.ts` | docs | XS | — | **next walk.ts touch** |
| Judgment-day theoretical: `create()`'s destination guard could additionally qualify by table (not just by path shape), closing a residual ambiguity between a path-collision and an option-shape rejection | edge-case | S | — | **next create() touch** |
| Judgment-day theoreticals (wording group): a NUL placeholder in walk-root rejection messages, and imprecise broken-symlink/file-as-source message wording, both cosmetic — no behavior change, message text only | docs | XS | — | **next path-guards/walk.ts touch** |
| Judgment-day theoreticals (edge-case group): whether `packageDir` itself being a symlink should be exempted from REQ-FSC-09.3's root-symlink rejection; a trailing `file/` on the `copyIn` wire form; `scaffold(undefined)` (no args object) should reject cleanly rather than throw a raw TypeError; an empty-string `rename` value's handling is unspecified | edge-case | S | — | **next scaffold-family touch** |
| Judgment-day theoretical: `globToRegex`'s per-call regex compilation has no cache bound — a pathological include/exclude pattern set could grow unbounded memory across a long-lived process (theoretical, no repro) | perf | XS | — | **next expander.ts touch** |
| Test-infra cleanup: the `m-17`/former `m-18` corpus fixture files carry stale internal names from the pre-renumber matrix (cosmetic, the manifest/citations are already correct); 9 now-dead `collection.json`-fabrication markers remain in test fixtures with no `fit-43` static-marker clause naming them as intentionally inert; `WalkEntry.absPath` is a dead field no consumer reads; one `CHANGELOG.md` entry line exceeds the file's own wrap convention | test-infra | S | — | **next scaffold-family/fixture touch** |
| `instrumentHarnessIO` order-tracking/failure-injection mode (simplify pass item A2, orchestrator-skipped as re-mechanizing verified security-critical shared infra) — would let canary/harness-opted-in spies assert call ORDER, not just occurrence | refactor | S | — | **next harness-instrumentation touch** |

Source: change `inline-collection-marker` archive (2026-07-29), commit range `1e44ae8..8c3828a` + archive-sync sync commit.

## From `runner-tripwire-invariants` judgment-day round 1 (2026-08-05) — scope correction, deferred soundness work

Two blind judges independently demonstrated **executable** bypasses of the capability-admission
mechanism (real `execSync("id")`, real `eval`, a `Function` construction printing the Node
version). That makes **three rounds** on the same root cause — the original build, a remediation
batch, and judgment day — each of which closed the spellings it was given while the next round
found new ones. The owner ruling was to stop claiming soundness and land honest scope: the
demonstrated repros are closed (see `apply-progress.md` §"Judgment-day remediation and scope
correction"), and the adversarial claims in ADR-0079/0080, `docs/runner-integrity-invariants.md`
and the guards' own doc comments are retracted rather than re-asserted.

**Read the head row first.** The remaining rows are evidence for a different mechanism, not a
queue of shapes to patch — the same meta-finding `runner-integrity-manifest`'s register already
recorded, now with a third round of confirming evidence and a named deliverable.

| Description | Type | Size | Gating? | Stage |
|---|---|---|---|---|
| **`capability-admission-oracle` — the deferred soundness work, as its own change.** Tracked upstream as [project-builder-sdk#65](https://github.com/Project-Builder-Schematics/project-builder-sdk/issues/65). Core deliverable: **`FIT-CAP-ORACLE`**, the differential oracle deferred in this change's `design.md` §7 — execute each corpus construct in a sandboxed realm and compare "did a capability actually become reachable" against the classifier's verdict, so the corpus is grown by the RUNTIME instead of by whoever last reviewed it. It is the one mechanism that would have caught all three rounds. Motivating evidence, each executed and each producing zero findings today: (1) a capability reached through a carrier property the deny predicate does not name — `const w = { go: globalThis }; const bad = w.go.Reflect.get(w.go, "eval"); bad("process.version")` prints the Node version; (2) the same through a getter or a class accessor; (3) an indexer function laundering the key — `function pick(o,k){return o[k]} const f = pick(globalThis,"eval"); f("process.version")`. **Explicit note for whoever picks this up: a member-path allowlist over an AST cannot be made sound without dataflow analysis.** ADR-0079 rejected the ts-morph type checker so a fail-closed gate's verdict would not depend on install state; that trade stands, which is exactly why the oracle (runtime evidence, no type information) is the shape of the answer rather than a wider allowlist. Do not spend another round closing spellings before this is decided | architecture | L | — | **own scoped change, before ANY further Constraint-4 shape work** |
| **JD-1** Safe-terminal path admission is a DENY predicate (`CAPABILITY_BEARING_SEGMENTS`), so `g().<anything-not-in-the-register>` passes. Closed for `eval`/`Function`/`binding`/`plugin`/`WebAssembly`/`register`/`registerHooks`/`constructor`/`__proto__`/`prototype`; the class is open. **Same class as the head row** | security | S | — | gated on `capability-admission-oracle` |
| **JD-2** A computed member access off a LOCAL root is admitted as ordinary indexing (36 such sites exist in the real closure and must stay admitted), which is what makes the indexer-function laundering in the head row work. Any fix must distinguish "indexing a local data structure" from "indexing a value that came from a global" — i.e. dataflow. **Same class as the head row** | security | S | — | gated on `capability-admission-oracle` |
| **JD-3** Enumeration totality is relative to the enumerator's five `SurfaceNodeKind`s: a construct it does not reach is invisible, not `unclassifiable-construct`. Tagged templates were such a construct for two rounds and are now enumerated; nothing structurally enumerates the *next* such form. FIT-CAP-TOTALITY's independent oracle mirrors the enumerator by hand, so it cannot detect a form both walkers miss | test-coverage | M | — | gated on `capability-admission-oracle` |
| **JD-4** `test/support/shared-build.ts` still builds into the REAL `dist/`, and `bun run build`'s `prebuild` deletes that tree recursively, so two concurrent `bun test` runs destroy each other's fixture (measured 2599/6 concurrent vs 2645/0 serial). Made LOUD in this pass via a pid-keyed owner lock (`.tmp-shared-build.lock`) rather than isolated: these checks exist to verify the tree that SHIPS, and a per-process scratch dist verifies a copy instead. Isolating properly means a scratch `outDir` + a scratch codegen outfile + a scratch manifest path, and re-deciding what `fit-42` is verifying | test-infra | M | — | **next `shared-build.ts` touch, or with `capability-admission-oracle`** |
| **JD-5** A shorthand property in an object literal was excluded from the surface as a "property name" (E3), whose justification — "the enclosing access is the surface node" — is false for shorthand. Fixed here (the exclusion now applies only where an enclosing access exists). The residue is doctrinal: `SURFACE_EXCLUSIONS` is pinned by exact membership, so each exclusion's *claim* is load-bearing, but nothing red-proofs the claim itself for `declaration-name`/`type-position` the way CAP-01.7 does for `jsdoc-rooted` | test-coverage | S | — | next `fit-42` touch |
| **JD-6** The judge cited `capability-admission.ts` ~583-585 as asserting `x()()` is undecidable, and reported that `function x(){return ()=>1} x()()` was admitted. **Not reproduced**: no such comment exists at that range, and corpus row c14 pins that exact source as one `constraint-4-undecidable-callee`, verified green. The over-claim actually present nearby — `resolveChain`'s "structurally incapable of naming an externally-sourced capability" for call-result terminals — was corrected instead, because a function returning `globalThis` refutes it. Recorded so the citation is not re-raised as an open finding | docs | XS | — | **closed, no action** |
| **JD-7** `checkSuiteGate`'s three failure reasons gained a `job "<name>": ` prefix when the check became per-job (a reason with no job name is ambiguous once several jobs carry publish steps). Pinned expectations updated in the same commit. If the engine or any external consumer greps these strings, they need telling | docs | XS | — | **next packaging-check touch** |
| **JD-8** `findNpmPublishCommandLines` now returns every `npm publish` line and the caller asserts `--dry-run` on all of them; `checkSuiteGate` and the ordering checks now loop all jobs. The remaining asymmetry: `checkPublishOrdering`/`checkExplicitRebuildStep` still reason about the FIRST stamp and FIRST publish step only, so a workflow with two stamps or two publishes is decided by one pair | test-coverage | S | — | **next packaging-check touch** |

Source: change `runner-tripwire-invariants`, judgment-day round 1 remediation + scope correction
