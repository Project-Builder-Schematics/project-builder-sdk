...
| **F-5** `dist/bin` build wiring for `pbuilder-runner` deferred to the public-package plan (ADR-0058 documented deviation — the bin currently produces no `dist/` output; `package.json#bin` entry also deferred) | refactor | S | Before any public release exposing the runner bin | **public-package plan** |
