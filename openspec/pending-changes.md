...
| Pre-existing `session.ts` behavior (verify-final council, architect): a `TransportFault` surfacing during `Session.flush()`'s emit leg is degraded by the `toAuthoringError` catch-all to exit code 2 (emit-rejection) instead of 3 (transport fault) — documented in `openspec/changes/stdio-engine-client/apply-progress.md:256-263`. Future `session.ts` changes touching the emit fault path should reclassify wire faults before the catch-all rather than rediscovering this from scratch | edge-case | S | — | **next `session.ts` touch** |
