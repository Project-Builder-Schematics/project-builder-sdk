...
## From `runner-tripwire-invariants` (2026-08-05)

### Three independent adversarial rounds on a spelling-based guard prove the mechanism is unsound, not just incomplete
**What**: Default-deny for capability admission was initially framed as a property (reject any unrecognized node), then tested via a deny-scan enumerating `Function`, `eval`, etc. Three independent adversarial rounds each closed the spellings they were shown — `const F = Function; F("...")`, `[].constructor.constructor`, `Reflect.get(globalThis,"eval")` — and each next round found three new forms unreachable from the previous round's understanding. Round 4 was pre-registered before a retraction moved the mechanism to the archive-ready state.
**Why**: An enumerating check answers "does this match something I forbade?", and its correctness depends entirely on the author's spelling imagination — it has no tail. Revisiting the same subsystem five times with fresh eyes and finding regressions every time is not bad luck; it is architecture telling you the mechanism choice is wrong. The reason was structural: "default deny except for [known shapes]" cannot bound an unbounded name space. The fix that survived was structural: flip to "deny by predicate" (PATH checked against a rule, not against an enumeration), making violation-by-default decidable.
**Where**: `scripts/capability-admission.ts` and `scripts/derive-runner-closure.ts`; `verify-report.md`'s C-1 analysis spells out the three root causes (taint not transitive, unresolvable free roots admitted in value position, `safe-terminal` as an unlisted fifth origin).
**Learned**: When an adversarial round finds regressions in the same family after a fix that looked complete, before another round of patching ask "am I enumerating into a space that has no enumeration?" — the answer usually points to a mechanism mistake, not a spelling gap. A second independent-judge round finding new shapes is the decision trigger, not evidence of effort needing to continue.

### Completeness and soundness are independent properties; red-proofs enforce completeness but not soundness
**What**: `FIT-CAP-TOTALITY` was designed to prove completeness (no node left unchecked) and was mistakenly read as proving soundness (no wrongly-admitted node). The change's north-star promised soundness as a side effect of closure integrity; the verify report retracted it after the third adversarial round, reframing as "drift control against honest mistakes", leaving soundness work for a registered successor (`capability-admission-oracle`, `FIT-CAP-ORACLE`).
**Why**: A red-proof that "mutating X breaks the test" proves the test is sensitive to X (completeness) but does not prove the test rejects all WRONG values of X (soundness) — those are different, and conflating them hides what the mechanism actually guarantees. The closure's 49 red-proofs (laundering corpus) proved every member was tested; they did not prove nothing untested could execute.
**Where**: `test/fitness/fit-42-runner-closure-integrity.test.ts`; `docs/runner-integrity-invariants.md#known-gaps` for the documented unsoundness (three residual bypasses).
**Learned**: When a spec promises a closed property, ask "is this completeness (I checked everything) or soundness (I correctly rejected everything wrong)?" and test explicitly against both. A test that proves "all 49 scenarios are exercised" and a test that proves "an unenumerated scenario fails" are both needed; one alone gives false confidence. Record both explicitly or retract the claim before archive.

### A prose count is not a gate; only an executed tally survives refactoring
**What**: The laundering corpus was disclosed as "26 constructs proved" in the change's own narrative. The actual count was 25 table rows + 1 separate entry = 26 total, but the tally device was a prose claim and a regex: `"49/49 red-proofs"` was measured by reading a single test file's count at one moment and carried verbatim through three review rounds. Mid-journey, the test's own count drifted to 47 as unrelated refactoring touched the file, undetected until the final verify gate re-measured it directly against the code.
**Why**: A count committed to prose is a one-time snapshot; every change to the file (new tests, removed tests, renamed constructs) invalidates it silently unless something re-checks it on every run. A test counting only double-quoted ids (`rg -c 'id: "'`) misses four single-quoted ids where the fixture text itself contains a double quote.
**Where**: `test/support/corpus-completeness.ts` and `fit-42-runner-closure-integrity.test.ts`; any change that makes a public disclosure of a count without backing it by a live assertion.
**Learned**: Never commit to a prose count without an executable assertion (`expect(LAUNDERING_CORPUS.length).toBe(26)`) that runs on every build. And when crafting a regex-based count, test against the actual data set first — a regex that looks right can silently miss edges when the test data changes. Even better: generate and test the count, never hand-count or regex-count and commit the result.

### `it.skipIf(root)` wrapping a whole `describe` loop silently deletesall tests inside, reported as green
**What**: Six tests guarding under `it.skipIf(process.getuid?.() === 0)` (root-run skip) were intended to skip individually, but one test block was organized as a `describe(...).forEach()` with all its `it()` calls nested inside — skipping at the test-registration level meant the entire loop was never invoked, so ALL tests inside it never ran, never registered, and never showed up as skipped in the suite summary. The root-privileged CI container ran those six tests silently missing, reported as green.
**Why**: `it.skipIf` is evaluated at **test registration time**, before the test names are enumerated. A `describe` that contains its own `it` registrations means "skip the describe" skips those registrations before they happen — structural visibility loss, not a test count.
**Where**: `test/fitness/fit-42-runner-closure-integrity.test.ts` (Amd B replaced `skipIf` wrapping with skip-recording fixtures, so the mechanism improved).
**Learned**: Never nest test registrations (`it(...)`) inside a loop-with-skip; if a root-skip is needed, register tests unconditionally and skip at the assertion level (`if (uid === 0) return;`) so the test name stays visible and audit trails show what was skipped and why. Skipping at registration time makes tests invisible to auditing and test counters.

### Archive-time registrations skip a cycle if not transcribed during the prior archive
**What**: Followups registered as pending changes owe a transcription step at each archive: copy the list from the design's "open items" section, check it against the prior cycle's archive register, and re-commit it. This cycle's change inherited a `capability-admission-oracle` line from the PREVIOUS cycle's archive register that was never transcribed to THIS cycle's design — it was ready to hand off, one grep away, but without the transcription step it never re-appeared, only to resurface at the final archive gate with a "re-register it" task.
**Why**: A followup in prose is easy to write and easy to forget; one in a structured register is scannable. Moving a pending item from one change's archive register to the next change's PROPOSAL is not automatic — it requires the orchestrator or steward to read the prior archive and explicitly carry it forward, or the change author to search backward for it at design time.
**Where**: `openspec/pending-changes.md`, `design.md`'s "Open Items" sections, and the prior-change's archive report.
**Learned**: At design time, before spec sign-off, run `grep -r "pending-changes.md" openspec/changes/archive/ | grep -i "{topic}"` for any pre-registered followups — search backward one cycle, not forward from memory. The discipline of scanning the prior archive before drafting open items catches the items an earlier phase registered and this change owes a public acknowledgment for.

Source: change `runner-tripwire-invariants` (2026-08-05)
