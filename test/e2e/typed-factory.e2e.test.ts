 ...
 * contract). Reject variant asserts the INTERIM plain-`Error` message (constraint 1); the
 * `instanceof AuthoringError`/origin/reason facets are S-006-only.
 */
...
describe("e2e — typed factory (schema-derived Input, run-boundary rejection)", () => {
  ...
  it("rejects a resolved input missing the required 'port' key BEFORE the factory body runs (RBV-01.1 site proof)", async () => {
    ...
    // INTERIM (S-000..S-005): a plain Error, never AuthoringError — constraint 1.
