...
import { checkVersionHasChangelogHeading } from "../../scripts/validate-release.ts";
...
describe("REQ-MFB-02.1 — package.json#version is bumped alongside the CHANGELOG", () => {
  ...
  it("the working-tree release version matches the CHANGELOG's topmost heading without stamp exemptions", () => {
    expect(checkVersionHasChangelogHeading(workingTreeVersion(), RELEASE_HEADING.exec(changelog())?.[1])).toEqual({
      ok: true,
    });
  });

  it("[red-proof] the guard fails on a mismatch and on a missing heading", () => {
